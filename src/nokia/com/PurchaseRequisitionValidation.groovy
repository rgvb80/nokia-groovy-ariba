...
def Message processData(Message message) {
    ...
    builder.'ns1:ARBCIG_DERIVATION'('xmlns:ns1':'urn:sap-com:document:sap:rfc:functions') {
        'ARIBA_DOC_NO'(AribaRequest.Requisition_ProcessReqExtHeaderDetails_Item.item.UniqueName)
        'PARTITION'(AribaRequest.@partition)
        'VARIANT'(AribaRequest.@variant)
        'SOURCE'("BUY")
        def String companyCode = AribaRequest.Requisition_ProcessReqExtHeaderDetails_Item.item.CompanyCode.UniqueName

        def validItems = AribaRequest.Requisition_ProcessReqExtLineDetails_Item.item.LineItems.item.findAll {
            item -> (item.LineType.Category.text() == '1' || item.LineType.Category.size() == 0)
        }
